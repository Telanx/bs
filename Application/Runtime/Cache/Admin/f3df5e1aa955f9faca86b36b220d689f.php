	...
	var popup=new pop(mask,container,title,body);
		...
		//清空
			...
			search_admin(k,c,p,function(res){
				...
				var r = res.r;
				for(var i=0;i<rl;i++){
					$('.search-result-table').append("<tr><td><button class='button border-dot btn-user-del'>删除</button><button class='button border-sub btn-user-edit'>修改</button></td><td><a href=# class='view'>"+r[i].user+'</a></td><td>'+(r[i].name||'-')+'</td><td>'+(r[i].status||'-')+'</td><td>'+(r[i].class||'-')+'</td></tr>');
				}
				//显示或者隐藏分页
					...
					<div class='right-body'>
						...
						<div class='right-result'>
							...
							<div class='right-result-body'>
								<table class="table table-hover search-result-table"><tr class='th'><th>操作</th><th>账号</th><th>姓名</th><th>状态</th><th>班级</th></tr>
								</table>
                                <!---start-->
